    ...
    return (
        <div className="border-2 border-gray-200 px-3 pt-3 pb-1 shadow-lg w-72 items-center hover:bg-gray-100 mb-2">
            <div className="flex items-center justify-between">
                ...
                <div className={`w-fit p-3 rounded-full font-semibold text-white ${item?.status == 'pending' && 'bg-yellow-500'} ${item?.status == 'accepted' && 'bg-green-500'} ${item?.status == 'rejected' && 'bg-red-500'}`}>{item?.status}</div>
            </div>
